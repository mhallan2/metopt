...
устойчивее для функций, на которых построение параболы временно ненадёжно.
...
на всём исходном интервале существует только один минимум.
...
длины шага сетки задаёт оценку погрешности по координате:
...
\begin{equation}
M_{\mathrm{new}} = C_{\mathrm{safety}} \cdot \max_i
\left|
...
на всём исходном интервале, точки вычислений и найденный минимум. На втором
...
Для визуализации используется два графика:
на первом отображается функция на всём интервале,
на втором --- увеличенная область около найденного минимума.
