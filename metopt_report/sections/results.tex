...
Вычислительные эксперименты проведены для трёх тестовых функций:
\begin{align*}
	f_1(x) &= (x-2)^2+1, \quad x\in[0,5],\\
	f_2(x) &= x^2+2\sin(3x), \quad x\in[0.8,3],\\
	f_3(x) &= x^2+5\sin(2x)+2\cos(3x), \quad x\in[-3,3].
\end{align*}
...
Для каждой локальной процедуры на рисунке показаны график функции на всём
...
графики подтверждают, что уменьшение числа вычислений достигается за счёт
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.88\textwidth]{results/comparison/global_x_error_f3.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.88\textwidth]{results/comparison/global_f_error_f3.png}
	\caption{Ошибка значения функции при одинаковом числе вычислений.}
	\label{fig:global-f-error}
\end{figure}

На этих графиках сравнивается не только итоговая ошибка, но и скорость её
уменьшения по мере расходования вычислительного бюджета. Для равномерного
перебора ошибка уменьшается постепенно за счёт уплотнения сетки. Метод ломаных
может быстрее уточнять перспективную область, но результат зависит от качества
начальной оценки $M$ и от распределения уже вычисленных точек.

\newpage

Отдельно выполнено сравнение локальных методов при одинаковом числе вычислений.
Результат представлен на рисунке~\ref{fig:local-comparison}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.88\textwidth]{results/comparison/local_f3.png}
	\caption{Сравнение длины итогового интервала при одинаковом числе вычислений.}
